...
function mostrarHistorial() {
    ...
    historial.forEach(function(h) {
        const item = document.createElement('historial-boton');
        item.setAttribute('fecha', h.fecha);
        item.setAttribute('puntaje', h.puntaje);
        item.setAttribute('total', h.total);
        contenedor.appendChild(item);
    });
}

document.addEventListener('click', async function(e) {
    if (e.target.id !== 'btn-jugar') return;
...
document.addEventListener('click', function(e) {
    if (e.target.id !== 'btn-volver') return;
    mostrarHistorial();       
    mostrarVista('vista-menu');
});
cargarCategorias();
